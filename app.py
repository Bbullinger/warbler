...
# With python 3.10 there is an attribute error "module-collections-has-no-attribute-mutablemapping"
# This block circumvents the issue until an update is issued
...
def users_show(user_id):
    ...
    # Grab id's of all followers and own id, to be used to query for messages to display to user
    following = [followers.id for followers in user.following] + [user.id]
    messages = (
...
####Create a route to 'like' a message, POST only
@app.route("/users/add_like/<int:msg_id>", methods=["POST"])
def add_like(msg_id):
    message = Message.query.get_or_404(msg_id)

    # Check if user had already liked message, if so, remove from likes
...
def homepage():
    ...
    if g.user:
        following = [followers.id for followers in g.user.following] + [g.user.id]
        messages = (
            Message.query.order_by(Message.timestamp.desc())
            .filter(Message.user_id.in_(following))
            .limit(100)
